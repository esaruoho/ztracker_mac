...
void CUI_PENature::draw(ZTDrawable *S)
{
    if (S->lock()==0)
    {
        int window_width = 40 * col(1);
        ...
        print(col(textcenter((char *)"Naturalization Amount")), start_y + row(2), (char *)"Naturalization Amount", COLORS.Text, S);
        print(start_x + col(1), start_y + window_height / 2, (char *)"  Percent:", COLORS.Text, S);
        UI->draw(S);
        S->unlock();
